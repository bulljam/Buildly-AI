...
export function ProjectBuilder({
  ...
  async function submitPrompt(rawPrompt: string) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(body && "error" in body ? body.error : "Generation failed.")
      }
      ...
      <section className="rounded-3xl border border-border/70 bg-gradient-to-br from-background via-background to-muted/60 p-6 shadow-sm">
        <div className="max-w-3xl space-y-3">
          ...
          <p className="text-sm leading-7 text-muted-foreground sm:text-base">
            Prompt the model to refine this site. The chat history stays attached to this project,
            and the preview only updates when a new valid HTML document is returned.
          </p>
        </div>
      </section>
